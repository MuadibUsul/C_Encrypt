 ...
 */
unsigned char *aes_init(size_t key_size) {
 ...
 */
void aes_cipher(unsigned char *in, unsigned char *out, unsigned char *w) {
 ...
 */
void aes_inv_cipher(unsigned char *in, unsigned char *out, unsigned char *w) {
...
void AES_ENCRYPT_MESSAGE(unsigned char message[32], unsigned char ciphertext[32]){
	...
	aes_cipher(message /* in */, ciphertext /* out */, w /* expanded key */);

	free(w);
}


void AES_DECRYPT_MESSAGE( unsigned char ciphertext[32], unsigned char message[32]){
	...
	aes_inv_cipher(ciphertext, message, w);
